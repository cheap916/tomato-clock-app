...
def main(page: ft.Page):
    ...
    # 🎵 播放列表 (根据你截图修改：只有 kanong.mp3)
    bgm_playlist = [
        {"name": "卡农", "src": "kanong.mp3"},
    ]
    current_bgm_index = 0

    emojis = {
        "idle": ["( =ω=)..zzZ", "( -ω-)", "₍ ᐢ. ̫ .ᐢ ₎", "(=^･ω･^=)"],
        "work": ["(ง •̀_•́)ง", "φ(．．;)", "(=`ω´=)", "( * >ω<)p"],
        "break": ["( ~ o ~ )~", "旦_(^O^ )", "☕(・ω・)", "🧴(舔毛中)"],
        "happy": ["(≧◡≦) ♡", "(=^･^=)♪", "(/ =ω=)/", "o(>ω<)o"],
    ...
    def weather_loop_thread():
        while True:
            ...
            weather_icon.name = random.choice([ft.Icons.PETS, ft.Icons.CLOUD, ft.Icons.WB_SUNNY])
            page.update()
            time.sleep(300)
    ...
    top_bar = ft.Row([
        ...
        ft.Row([btn_bgm, ft.IconButton(icon=ft.Icons.SKIP_NEXT, icon_size=16, icon_color=THEME["fg"],
                                       on_click=lambda e: page.snack_bar.open(
                                           ft.SnackBar(ft.Text("只有一首歌哦喵~"), open=True)) and page.update())],
               spacing=0),
        btn_checkin
