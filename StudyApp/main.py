...
class StudyLogic:
    ...
    def increment_tomato(self):
        ...
        time_str = datetime.now().strftime("%H:%M")
        self.data["history"].append(f"[{time_str}] 捕获一只番茄 🍅 (嚼嚼���)")
        self.save_data()
...
def main(page: ft.Page):
    ...
    audio_bg = flet_audio.Audio(src=bgm_playlist[0]["src"], autoplay=False, volume=1.0, release_mode="loop")
    ...
    def toggle_bgm(e):
        ...
        if bgm_enabled:
            ...
            audio_bg.volume = 1.0  # 恢复音量
        else:
            btn_bgm.icon = ft.Icons.MUSIC_OFF
            btn_bgm.tooltip = "背景音: 关闭"
            audio_bg.volume = 0.0  # 设为静音，但不要 pause()!

        audio_bg.update()

        # 如果正在计时，强制 resume 以防万一
        if timer_running:
            audio_bg.resume()
    ...
    # --------------------------------------------------------
    # 核心修改：toggle_timer
    # 无论 bgm_enabled 是 True 还是 False，都必须播放音频！
    # --------------------------------------------------------
    def toggle_timer(e):
        ...
        if not timer_running:
            ...
            # 🛑 关键修复：强制播放背景音
            # 如果 bgm_enabled=False，前面 toggle_bgm 会把 volume 设为 0
            # 所以这里 resume() 会播放静音，但能保活！
            try:
                audio_bg.resume()
            except:
                pass
