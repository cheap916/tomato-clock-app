...
def main(page: ft.Page):
    ...
    page.padding = 0
    page.keep_screen_on = True  # 🌟 屏幕常亮 (防止系统杀后台的第一道防线)
    ...
    bgm_playlist = [
        {"name": "呼噜噜", "src": "assets/silent.mp3"},
    ...
    # 🔊 音频初始化 (release_mode="loop" 确保后台不被杀)
    ...
    def send_notification(title, message):
        def _notify():
            try:
                for i in range(3):  # 连续提醒 3 次
                    notification.notify(
                        title=f"{title} (提醒 {i + 1})",
                        message=message,
                        app_name="猫猫专注",
                        timeout=10
                    )
                    time.sleep(1.5)  # 间隔 1.5 秒
            except:
                pass
    ...
    def toggle_bgm(e):
        ...
        if bgm_enabled:
            ...
            btn_bgm.tooltip = "静音 (注意：可能导致后台失效)"
            try:
                audio_bg.pause()
            except:
                pass
